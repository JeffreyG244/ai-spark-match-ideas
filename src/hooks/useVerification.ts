  ...
  const fetchVerificationStatus = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('phone_verified, photo_verified, email_verified, phone_verified_at, photo_verified_at, email_verified_at')
        .eq('user_id', user.id)
        .single();
  ...
  const fetchPhoneVerification = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('phone_number, verified_at, code_expires_at')
        .eq('user_id', user.id)
