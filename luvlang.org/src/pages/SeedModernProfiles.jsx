...
const SeedModernProfiles = () => {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-love-background via-white to-pink-50 p-6">
      <div className="max-w-2xl mx-auto">
        <Card className="border-love-primary/20 shadow-xl">
          ...
          <CardContent className="space-y-6">
            {!isSeeding && !isComplete && (<div className="text-center space-y-4">
                ...
                <Button onClick={handleSeedProfiles} className="w-full bg-gradient-to-r from-love-primary to-love-secondary hover:from-love-primary/90 hover:to-love-secondary/90 text-white" size="lg">
                  <Sparkles className="h-5 w-5 mr-2"/>
                  Seed Modern Profiles
                </Button>
              </div>)}

            {isSeeding && (<div className="text-center space-y-4">
            ...
            {isComplete && results && (<div className="text-center space-y-4">
                {results.success ? (<>
                    <CheckCircle className="h-12 w-12 text-green-600 mx-auto"/>
                    <div>
                      <h3 className="text-xl font-bold text-green-800 mb-2">Success! 🎉</h3>
                      <p className="text-gray-700 mb-4">{results.message}</p>
                      {results.count && (<div className="bg-green-50 border border-green-200 rounded-lg p-4">
                          <p className="text-green-800 font-semibold">
                            {results.count} new profiles added to your dating app!
                          </p>
                        </div>)}
                    </div>
                    <Button onClick={() => window.location.href = '/discover'} className="bg-gradient-to-r from-love-primary to-love-secondary hover:from-love-primary/90 hover:to-love-secondary/90 text-white">
                    ...
                    setIsComplete(false);
                    setResults(null);
                }} variant="outline" className="border-love-primary text-love-primary hover:bg-love-primary hover:text-white">
                      Try Again
                    </Button>
                  </>)}
              </div>)}
