export const calculateCompatibilityScore = (user1, user2) => {
    ...
    if (user1.values && user2.values) {
        ...
        const commonValues = value1Words.filter(word => value2Words.includes(word) && word.length > 3);
    ...
    if (user1.lifeGoals && user2.lifeGoals) {
        ...
        const commonGoals = goals1Words.filter(word => goals2Words.includes(word) && word.length > 3);
...
export const generateDailyMatches = (currentUser, allUsers, excludeUserIds = [], maxMatches = 10) => {
    const eligibleUsers = allUsers.filter(user => user.id !== currentUser.id &&
...
export const getMatchInsights = (matchScore) => {
    const insights = [];
