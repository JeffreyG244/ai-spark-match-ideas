...
export const useCriticalSecurity = () => {
    ...
    const validateCriticalPassword = useCallback(async (password) => {
        setIsValidating(true);
        try {
            const validation = validatePasswordAgainstLeaks(password);
            // Log if password is compromised
            if (!validation.isSecure) {
                await logCriticalSecurityEvent('compromised_password_attempt', {
    ...
    const checkCriticalRateLimit = useCallback((action, maxAttempts) => {
        const key = `${action}_${Date.now().toString(36)}`;
        return criticalRateLimit.checkAllowed(key, maxAttempts);
    }, []);
    const reportSecurityIncident = useCallback(async (incidentType, details) => {
        await logCriticalSecurityEvent(`incident_${incidentType}`, details, 'critical');
        toast({
