    ...
    const loadMessages = async () => {
        ...
        try {
            ...
            if (data && previousMessagesCountRef.current > 0 &&
                ...
                logSecurityEvent('message_injection_suspected', `Abnormal message count increase in conversation ${conversationId}`, 'high');
            }
            ...
            try {
                ...
                channelRef.current = supabase
                    ...
                    if (!payload.new || typeof payload.new !== 'object' || !payload.new.id) {
                        logSecurityEvent('invalid_message_received', `Invalid message structure received in conversation ${conversationId}`, 'medium');
                        return;
                    }
                    ...
                    if (!payload.new || typeof payload.new !== 'object' || !payload.new.id) {
                        logSecurityEvent('invalid_message_update', `Invalid message update received in conversation ${conversationId}`, 'medium');
                        return;
                    }
