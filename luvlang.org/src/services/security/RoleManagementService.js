...
export class RoleManagementService {
    ...
    static async assignUserRole(userId, role) {
        try {
            const { data: { user } } = await supabase.auth.getUser();
            if (!user) {
            ...
            if (!validationResult) {
                await SecurityAuditService.logSecurityEvent('unauthorized_role_assignment_attempt', {
                    assigner_id: user.id,
                    target_user_id: userId,
                    attempted_role: role
                }, 'high');
            ...
            await SecurityAuditService.logSecurityEvent('user_role_assigned', { target_user_id: userId, role, granted_by: user.id }, 'medium');
        }
        catch (error) {
