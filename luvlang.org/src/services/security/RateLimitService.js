...
export class RateLimitService {
    ...
    static isAllowed(key, maxRequests, windowMs) {
        ...
        if (validRequests.length >= maxRequests) {
            SecurityAuditService.logSecurityEvent('client_rate_limit_exceeded', { key, maxRequests, windowMs, currentCount: validRequests.length }, 'medium');
            return false;
        }
    ...
    static async checkAnonymousRateLimit(endpoint) {
        try {
            ...
            const recentRequests = requests.filter((timestamp) => timestamp > windowStart.getTime());
            if (recentRequests.length >= 10) {
                await SecurityAuditService.logSecurityEvent('anonymous_rate_limit_exceeded', {
                    endpoint,
                    identifier,
